#include "../include/cpu_map_ui16.hpp"

CpuMapUI16::CpuMapUI16()
{
    this->size_x = 0;
    this->size_y = 0;
}

CpuMapUI16::CpuMapUI16(int size_x, int size_y)
{
    this->resize(size_x, size_y);
}


CpuMapUI16::CpuMapUI16(int size_x, int size_y, const uint16_t fill_value)
{
    this->resize(size_x, size_y);
...
void CpuMapUI16::allocate(int size_x, int size_y)
{
    this->size_x = size_x;
    this->size_y = size_y;

    this->data = (uint16_t*) malloc(size_x * size_y * sizeof(uint16_t));
...
void CpuMapUI16::resize(int size_x, int size_y)
{
    release();
...
void CpuMapUI16::fill(const uint16_t fill_value)
{
...
void CpuMapUI16::display(std::string win_name)
{
